...
// Лучше импортировать classNames вместо cn, так нагляднее
...
// Переусложнил интерфейс, можно было просто написать обычный интерфейс DetailProps
...
export async function getStaticProps({ params }: GetStaticPropsContext) {
	...
	// Тут можно не указывать тип, потому что ты его нигде не объявляешь и он будет any
	const { data: catalogData } = await axios.get(API.catalogCards.url);
	const { catalogCards } = catalogData;

	const card = catalogCards.find(
		({ slug }: DetailPageProps) => slug === params.detail
	);
	...
	// Лучше деструктуризировать, так будет нагляднее, что приходит
	return {
		props: {
			...card,
		},
